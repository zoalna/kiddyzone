...
{!! view_render_event('bagisto.shop.products.review.before', ['product' => $product]) !!}
    ...
    @if ($total)
        ...
        @else
            ...
            <div class="customer-reviews col-6 pt-5">
                ...
                @foreach ($reviews as $review)
                    <div class="rows">
                        <!-- <h4 class="col-lg-12 fs18">{{ $review->title }}</h4> -->
                        <div class="reviwe-content col-12 row pl-0 mb-5">
                            <div class="reviwe-left col-4 pl-0">
                                <div class="image col-lg-12">
                                    @if (count($review->images) > 0)
                                    @foreach ($review->images as $image)
                                        <img class="image" src="{{ $image->url }}" style="height: 96px; width: 97px; margin: 5px;">
                                    @endforeach
                                    @endif
                                </div>
        ...
        @else
            <div class="row customer-rating col-6">
                <div class="row full-width text-center product-rating-custome">
                    <div class="col-lg-12 col-xl-6">
                        <h3 class="col-lg-12 productc">{{ $avgRatings }}</h3>

                        <star-ratings
                            :size="24"
                            :ratings="{{ $avgStarRating }}"
                        ></star-ratings>
                        <h4 class="custom-text">Average Rating</h4>
                    ...
                    <div class="col-lg-12 col-xl-6">
                        ...
                        @for ($i = 5; $i >= 1; $i--)
                            ...
                            <div class="row custome-star">
                                <span class="col-3 no-padding fs16 fw6 custome">{{ $i }} </i><i class="material-icons ">star</i></span>
                                <div class="col-7 rating-bar" title="{{ $percentageRatings[$i] }}%">
                                    <div style="width: {{ $percentageRatings[$i] }}%"></div>
                                </div>
