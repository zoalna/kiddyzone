...
class Helper extends Review
{
     ...
     */
    /**
     * Get shop Hot Sellers Products.
     *
     * @param  int  $reviewCount
     * @return \Illuminate\Support\Collection
     */
    public function getHotSellersProducts($reviewCount = 5)
    {
        $products = $this->productRepository
        ->join('order_items', 'order_items.product_id', '=', 'products.id')
        ->selectRaw('products.*, SUM(order_items.qty_ordered) AS quantity_sold')
        ->groupBy(['products.id']) // should group by primary key
        ->orderBy('qty_ordered', 'desc')
        ->take($reviewCount) // 20 best-selling products
        ->get();
